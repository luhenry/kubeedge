...
RUN if [ "${ARCH}" = "amd64" ]; then apt-get install -y gcc-aarch64-linux-gnu libc6-dev-arm64-cross gcc-arm-linux-gnueabihf gcc-riscv64-linux-gnu libc6-dev-riscv64-cross ; fi
...
RUN if [ "${ARCH}" = "amd64" ]; then wget -c https://dl.google.com/go/go1.23.12.linux-amd64.tar.gz -O - | tar -xz -C /usr/local; \
    ...
    elif [ "${ARCH}" = "riscv64" ]; then wget -c https://dl.google.com/go/go1.23.12.linux-riscv64.tar.gz -O - | tar -xz -C /usr/local; \
    fi
